...
Custom middleware that hides components only from listings (in `on_list_prompts`, say) hides them from listings only, as it does for `prompts/get`. To hide them from completion too, implement [`on_complete`](/servers/middleware#on_complete). If the candidates themselves should be restricted per caller, check the auth context inside the handler (via [`get_context()`](/servers/context)) and return `None` when the caller is not permitted.
