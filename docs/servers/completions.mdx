...
For a [mounted server](/servers/composition), completion runs the parent's middleware and applies the mounted server's visibility and component `auth=`, but it does not run the mounted server's own middleware. A component hidden only by middleware on the mounted server, including its `AuthMiddleware`, still completes. Put that middleware on the parent to cover completion.
