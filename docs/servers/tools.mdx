...
<ParamField body="title" type="str | None">
  A human-readable display title for the tool. If omitted, FastMCP falls back to `annotations.title` when present, then to a title derived from the tool's name (e.g. `find_products` becomes "Find Products") — some MCP clients drop tools that have no title at all. An explicit `title` is also copied into `annotations.title` when that field is unset, since some clients and the Claude connectors directory read the title from there.
</ParamField>
...
See the [Docket documentation](https://docket.lol/en/latest/task-behaviors/#task-timeouts) for more on task timeouts and retries.
</Warning>
